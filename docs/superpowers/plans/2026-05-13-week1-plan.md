...
**Goal:** Stand up a FastAPI service backed by PostgreSQL + pgvector that ingests ~100 USC-area cafes from Google Places and Yelp Fusion, enriches each with LLM-extracted attributes and an embedding, and exposes a `POST /search` endpoint that returns the top 5 cafes for a natural-language query.
...
- [ ] `GOOGLE_PLACES_API_KEY` available (see spec §3.a)
- [ ] `YELP_API_KEY` available (see spec §3.b) — if still pending, you can complete Tasks 0–4 and 9–11, then circle back to 5–8 once Yelp approves
...
class Cafe(Base):
    ...
    yelp_business_id: Mapped[str | None] = mapped_column(String, unique=True, nullable=True)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("yelp_business_id", sa.String(), nullable=True),
...
## Task 3: Yelp Fusion Client

**Files:**
- Create: `backend/app/ingestion/yelp.py`
- Create: `backend/tests/unit/test_yelp.py`
- Create: `backend/tests/fixtures/yelp_business_search.json`
- Create: `backend/tests/fixtures/yelp_business_details.json`
- Create: `backend/tests/fixtures/yelp_reviews.json`
...
```json
{
  "businesses": [
    {
      "id": "bricks-scones-la",
      "alias": "bricks-and-scones-los-angeles",
      "name": "Bricks & Scones",
      "rating": 4.5,
      "review_count": 980,
      "price": "$$",
      "categories": [{"alias": "cafe", "title": "Cafes"}, {"alias": "bakeries", "title": "Bakeries"}],
      "location": {"address1": "403 N Larchmont Blvd"},
      "coordinates": {"latitude": 34.0751, "longitude": -118.3231}
    }
  ]
}
```
...
```json
{
  ...
  "review_count": 980,
  "price": "$$",
  "attributes": {"free_wifi": "yes"},
  "categories": [{"alias": "cafe", "title": "Cafes"}]
}
```
...
{
  "reviews": [
    {"id": "r1", "rating": 5, "text": "Super quiet and lots of outlets to plug in laptops."},
    {"id": "r2", "rating": 4, "text": "Wifi was fast. Great spot to work."},
    {"id": "r3", "rating": 5, "text": "Came on a Sunday morning—perfect for reading."}
  ]
}
```
...
from app.ingestion.yelp import YelpClient

FIXTURES = Path(__file__).parent.parent / "fixtures"


@pytest.mark.asyncio
async def test_business_search_returns_results():
    search = json.loads((FIXTURES / "yelp_business_search.json").read_text())
    with respx.mock(base_url="https://api.yelp.com") as mock:
        mock.get("/v3/businesses/search").mock(return_value=httpx.Response(200, json=search))
        async with YelpClient(api_key="fake") as client:
            results = await client.search(term="Bricks & Scones", lat=34.075, lng=-118.323)
    assert len(results) == 1
    r = results[0]
    assert r.yelp_business_id == "bricks-scones-la"
    assert r.price_level == 2  # "$$" → 2
    assert r.rating == 4.5
    assert r.review_count == 980


@pytest.mark.asyncio
async def test_reviews_returns_text_list():
    reviews = json.loads((FIXTURES / "yelp_reviews.json").read_text())
    with respx.mock(base_url="https://api.yelp.com") as mock:
        mock.get("/v3/businesses/bricks-scones-la/reviews").mock(
            return_value=httpx.Response(200, json=reviews)
        )
        async with YelpClient(api_key="fake") as client:
            review_texts = await client.reviews("bricks-scones-la")
    assert len(review_texts) == 3
    assert "outlets" in review_texts[0].lower()
```

- [ ] **Step 3: Verify they fail**

```bash
uv run pytest tests/unit/test_yelp.py -v
```
...
def _from_raw(raw: dict[str, Any]) -> YelpBusiness:
    return YelpBusiness(
        yelp_business_id=raw["id"],
        name=raw.get("name", ""),
        rating=raw.get("rating"),
        review_count=raw.get("review_count"),
        price_level=_PRICE_MAP.get(raw.get("price", "")),
        categories=[c["alias"] for c in raw.get("categories", [])],
        lat=raw.get("coordinates", {}).get("latitude", 0.0),
        lng=raw.get("coordinates", {}).get("longitude", 0.0),
    )
...
class YelpClient:
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(min=1, max=8))
    async def search(
        self, *, term: str, lat: float, lng: float, radius_m: int = 100, limit: int = 5
    ) -> list[YelpBusiness]:
        params = {
            "term": term,
            "latitude": lat,
        ...
        r = await self._client.get("/v3/businesses/search", params=params)
        r.raise_for_status()
        return [_from_raw(b) for b in r.json().get("businesses", [])]

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(min=1, max=8))
    async def reviews(self, business_id: str) -> list[str]:
        r = await self._client.get(f"/v3/businesses/{business_id}/reviews")
        r.raise_for_status()
        return [rv.get("text", "") for rv in r.json().get("reviews", [])]
```
...
```bash
cd ..
git add backend/app/ingestion/yelp.py backend/tests/unit/test_yelp.py backend/tests/fixtures/yelp_*
git commit -m "feat(ingestion): yelp fusion client (search + reviews)"
```

---

## Task 4: Reconcile (fuzzy match + merge)
...
from app.ingestion.reconcile import distance_m, is_same_place, merge
...
def test_distance_m_zero():
    assert distance_m(34.0, -118.0, 34.0, -118.0) == pytest.approx(0.0, abs=1e-6)


def test_distance_m_small():
    # 0.0001 deg lat ≈ 11.1 m
    d = distance_m(34.0, -118.0, 34.0001, -118.0)
    assert 10 < d < 13


def test_is_same_place_true_when_similar_name_and_close():
    g = _make_place(name="Bricks & Scones", lat=34.075, lng=-118.323)
    y_name = "Bricks and Scones"
    y_lat, y_lng = 34.07505, -118.32305
    assert is_same_place(g, y_name=y_name, y_lat=y_lat, y_lng=y_lng) is True


def test_is_same_place_false_when_name_wildly_off():
    g = _make_place(name="Bricks & Scones")
    assert is_same_place(g, y_name="Starbucks", y_lat=34.075, y_lng=-118.323) is False


def test_is_same_place_false_when_too_far():
    g = _make_place(lat=34.075, lng=-118.323)
    assert is_same_place(g, y_name="Bricks & Scones", y_lat=34.10, y_lng=-118.30) is False


def test_merge_overrides_with_yelp_rating_and_price():
    g = _make_place(rating=4.0, price_level=1, review_count=100)
    merged = merge(
        g,
        yelp_business_id="bricks-scones-la",
        yelp_rating=4.5,
    ...
    assert merged.review_count == 980
```

- [ ] **Step 2: Verify it fails**

```bash
uv run pytest tests/unit/test_reconcile.py -v
```
...
NAME_SIMILARITY_THRESHOLD = 80  # 0-100 from rapidfuzz token_set_ratio
...
def is_same_place(g: NormalizedPlace, *, y_name: str, y_lat: float, y_lng: float) -> bool:
    name_sim = fuzz.token_set_ratio(g.name, y_name)
    d = distance_m(g.lat, g.lng, y_lat, y_lng)
    return name_sim >= NAME_SIMILARITY_THRESHOLD and d <= DISTANCE_THRESHOLD_METERS


@dataclass
class MergedCafe(NormalizedPlace):
...
def merge(
    ...
    *,
    ...
    yelp_price_level: int | None,
) -> MergedCafe:
    merged = MergedCafe(
        **{f.name: getattr(g, f.name) for f in g.__dataclass_fields__.values()},
        yelp_business_id=yelp_business_id,
    )
    if yelp_rating is not None:
        merged = replace(merged, rating=yelp_rating)
    if yelp_review_count is not None:
        merged = replace(merged, review_count=yelp_review_count)
    if yelp_price_level is not None:
        merged = replace(merged, price_level=yelp_price_level)
    return merged
```
...
```bash
cd ..
git add backend/app/ingestion/reconcile.py backend/tests/unit/test_reconcile.py
git commit -m "feat(ingestion): reconcile fuzzy match and merge for google+yelp"
```
...
from app.ingestion.yelp import YelpBusiness
from app.models import Cafe


@pytest.mark.asyncio
async def test_pipeline_persists_enriched_cafes(session, monkeypatch):
    google_place = NormalizedPlace(
        ...
        reviews=["Quiet study spot with plenty of outlets."],
    )
    ...
    yelp_client = MagicMock()
    yelp_client.search = AsyncMock(return_value=[yelp_business])
    yelp_client.reviews = AsyncMock(return_value=["Wifi is fast.", "Lots of outlets."])
    ...
    assert row.yelp_business_id == "bricks-scones-la"
    assert row.has_outlet is True
    assert row.rating == 4.5  # yelp overrode google
...
from app.ingestion.google_places import GooglePlacesClient, NormalizedPlace
from app.ingestion.reconcile import is_same_place, merge
...
REVIEW_CACHE_DIR = Path("data/reviews_raw")


async def _gather_reviews(
    yelp_client, yelp_business_id: str | None, google_reviews: list[str]
) -> list[str]:
    reviews = list(google_reviews)
    if yelp_business_id:
        try:
            yelp_reviews = await yelp_client.reviews(yelp_business_id)
            reviews.extend(yelp_reviews)
            REVIEW_CACHE_DIR.mkdir(parents=True, exist_ok=True)
            (REVIEW_CACHE_DIR / f"{yelp_business_id}.json").write_text(json.dumps(yelp_reviews))
        except Exception as e:
            log.warning("yelp_reviews_failed", id=yelp_business_id, error=str(e))
    return reviews
...
async def run_pipeline(
    *,
    ...
    for raw in nearby:
        ...
        yelp_rating = yelp_review_count = yelp_price = None
        try:
            candidates = await yelp_client.search(term=details.name, lat=details.lat, lng=details.lng)
            for c in candidates:
                if is_same_place(details, y_name=c.name, y_lat=c.lat, y_lng=c.lng):
                    yelp_id = c.yelp_business_id
                    yelp_rating = c.rating
                    yelp_review_count = c.review_count
                    yelp_price = c.price_level
                    break
        except Exception as e:
            log.warning("yelp_search_failed", name=details.name, error=str(e))
        ...
        merged = merge(
            ...
            yelp_price_level=yelp_price,
        )

        reviews = await _gather_reviews(yelp_client, yelp_id, details.reviews)
        tags = await tag_cafe(
        ...
        log.info("cafe_upserted", name=merged.name, has_outlet=tags.has_outlet)
...
def main(lat: float | None, lng: float | None, radius_m: int | None, limit: int | None) -> None:
    ...
    async def _go() -> None:
        async with (
            GooglePlacesClient(api_key=settings.google_places_api_key) as g,
            YelpClient(api_key=settings.yelp_api_key) as y,
        ):
...
**Prerequisites:** all three API keys live; spec §10 Week 1 row is your pass/fail bar.

- [ ] **Step 1: Confirm API keys**

```bash
cd backend
grep -E '^(GOOGLE_PLACES|YELP|OPENAI)_API_KEY' .env | wc -l
```
...
- Ingestion phases 1-4 (Google fetch → Yelp reconcile → reviews → LLM extract + embed) → Tasks 2, 3, 4, 5, 6, 7
...
- **API key spend.** Tasks 1-10 cost $0 (everything is mocked). Task 11's smoke script costs ~$0.0001. Task 12 (real run) costs ~$0.50 for the LLM extraction + embeddings of 100 cafes.
- **If Yelp is still pending approval** when you start, run with `YELP_API_KEY=` empty: the pipeline already handles Yelp failures gracefully (logs a warning, continues with Google-only data). Re-run after Yelp is live to backfill `yelp_business_id`, prices, and reviews.
